...
use CsvMigrations\FieldHandlers\Renderer\Value\ListRenderer;
...
class ListRendererTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testRenderValue()
    {
        ...
        $result = $this->renderer->renderValue('parent', ['listItems' => $listItems]);
        $expected = 'Parent';
        $this->assertEquals($expected, $result, "Value rendering is broken for parent list value");

        $result = $this->renderer->renderValue('parent.child', ['listItems' => $listItems]);
        $expected = 'Parent:Child';
        $this->assertEquals($expected, $result, "Value rendering is broken for child list value");

        $result = $this->renderer->renderValue('parent.child.grandchild', ['listItems' => $listItems]);
    ...
    {
        $result = $this->renderer->renderValue('test', ['listItems' => 'not_an_array']);
    }
}
